...
def find_weekends(data):
    ...
    for day in data['DATETIME']:
        ...
        if weekend_found:
            if (index - start_weekend_list[-1]) >= 16: #16 data points per weekend for GFS 10day
                weekend_found = False
        index = index + 1
...
def Alert_GFS_10day(location):
    GFS_10DAY = "https://spotwx.com/products/grib_index.php?model=gem_glb_15km&lat=XXXX&lon=YYYY&tz=America/Edmonton&display=table"
